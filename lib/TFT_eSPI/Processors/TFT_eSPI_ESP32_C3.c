...
//*
void TFT_eSPI::pushBlock(uint16_t color, uint32_t len){
  // 补丁：arduino-esp32 3.x 下 SPIClass 独占 SPI2 外设，寄存器级直推
  //（*_spi_cmd = SPI_USR 忙等待）永远不会结束 → 改用 SPIClass 同步写。
  // 线上字节序与原实现一致：每像素先高字节后低字节（16 位色 MSB first）。
  uint8_t colorBin[2];
  colorBin[0] = (uint8_t)(color >> 8);
  colorBin[1] = (uint8_t)color;
  while (len) {
    uint32_t chunk = len > 4095 ? 4095 : len;
    spi.writePattern(colorBin, 2, chunk);
    len -= chunk;
  }
}
//*/
...
void TFT_eSPI::pushSwapBytePixels(const void* data_in, uint32_t len){
  ...
  // 线上字节序与原实现一致（DAT8TO32 写入 W 寄存器 → FIFO 先发低字节）：
  // 每像素两字节互换，即内存 [A,B] → 线上 [B,A]。
  const uint8_t* data = (const uint8_t*)data_in;
  static uint8_t buf[512];   // 一次最多 256 像素
  while (len) {
...
void TFT_eSPI::pushPixels(const void* data_in, uint32_t len){
  ...
  // 补丁：改用 SPIClass（原寄存器直推在 core 3.x 下挂死）。
  // 线上字节序与原实现一致：内存顺序原样发出
  //（uint32 写入 W 寄存器 → FIFO 先发低字节）
  spi.writeBytes((const uint8_t*)data_in, len * 2);
}
