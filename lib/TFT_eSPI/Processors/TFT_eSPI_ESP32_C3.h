...
#if !defined (ESP32_PARALLEL)
  // 补丁：arduino-esp32 3.x 下 SPIClass 独占 SPI2 外设，禁止直写 USER 寄存器
  //（原宏 *_spi_user = SPI_USR_MOSI 会与驱动状态冲突）
...
// 补丁：arduino-esp32 3.x 下 SPI 传输由 SPIClass 同步完成（transfer/write 返回即完成），
// 无需再轮询 CMD 寄存器 —— 且直写 CMD 寄存器会与驱动冲突导致 TG1WDT 挂死
...
//* Replacement slimmer macros
  ...
  #else
    // 补丁：改用 SPIClass（原寄存器直推在 core 3.x 下挂死）。
    // 原实现 *_spi_w = D 先发低字节，这里逐字节复刻线上顺序
    #define TFT_WRITE_BITS(D, B) \
      if ((B) == 16) { spi.transfer((uint8_t)(D)); spi.transfer((uint8_t)((D) >> 8)); } \
      else if ((B) == 32) { spi.transfer((uint8_t)(D)); spi.transfer((uint8_t)((D) >> 8)); \
                            spi.transfer((uint8_t)((D) >> 16)); spi.transfer((uint8_t)((D) >> 24)); } \
      else { spi.transfer((uint8_t)(D)); }
  #endif
  // Write 8 bits
  #define tft_Write_8(C) TFT_WRITE_BITS(C, 8)
  ...
  // Future option for transfer without wait
    ...
    #define tft_Write_16N(C) *_spi_mosi_dlen = 16-1;    \
                           ...
                           *_spi_cmd = SPI_USR;
  #else
    // 补丁：SPIClass 同步传输，线上字节序与原实现一致 [C_hi, C_lo]
    #define tft_Write_16N(C) { spi.transfer((uint8_t)((C) >> 8)); spi.transfer((uint8_t)(C)); }
  #endif
